...
**v2 (production)** と **v3 (dev)** をタブで切り替え可能。
...
| タブ | 用途 | エントリ |
|---|---|---|
| **v2 (production)** | 出荷中の Tube V2 用 Visualizer + Firmware Installer | [v2.html](v2.html) |
| **v3 (dev)** | 検証ツール群 (Face & Sound Detection 等) | [v3/face-sound.html](v3/face-sound.html) |

v3 はサブツール選択 dropdown を持ち、将来追加のツールを順次掲載。  
タブ切替時は **iframe を隠す/表示** で実装 → Web Serial 接続や状態が **タブ移動で失われない**。
...
| ファイル | 役割 |
|---|---|
| `index.html` | タブシェル (v2 / v3 切替 + iframe ホスト) |
| `v2.html` | v2 Visualizer + Firmware Installer (旧: index.html) |
| `v3/face-sound.html` | v3 dev: Face & Sound Detection (XIAO ESP32-S3 Sense) |
...
各 iframe は `allow="serial"` 付き同 origin 配信 → 各タブが独立した Web Serial port を持てる。
